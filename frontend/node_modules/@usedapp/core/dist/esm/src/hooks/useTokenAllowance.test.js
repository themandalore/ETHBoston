...
describe('useTokenAllowance', () => {
    ...
    it('returns 0 when spender is not yet approved', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useTokenAllowance(token.address, deployer.address, spender.address), {
            config,
        });
    ...
    it('multichain calls return correct initial balances', async () => {
        await testMultiChainUseTokenAllowance(token, deployer.address, spender.address, network1.chainId, MOCK_TOKEN_INITIAL_BALANCE.toString());
        await testMultiChainUseTokenAllowance(secondToken, secondDeployer.address, secondSpender.address, network2.chainId, SECOND_MOCK_TOKEN_INITIAL_BALANCE.toString());
    });
    const testMultiChainUseTokenAllowance = async (contract, user, spenderUser, chainId, allowance) => {
        ...
        const { result, waitForCurrent } = await renderDAppHook(() => useTokenAllowance(contract.address, user, spenderUser, { chainId }), {
            config,
        });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
