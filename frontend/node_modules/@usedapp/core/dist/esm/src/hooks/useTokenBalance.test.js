...
import { deployMockToken, renderDAppHook, setupTestingConfig } from '../testing';
...
describe('useTokenBalance', () => {
    ...
    it('returns balance for explicitly another chain', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useTokenBalance(token2.address, receiver, { chainId: network2.chainId }), { config });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
