...
describe('Multicall encoder', () => {
    ...
    it('Properly encodes', () => {
        const calldata = constants_1.ethersAbi.encodeFunctionData('aggregate', [calls.map((calldata) => [address, calldata])]);
        const manual = (0, encoder_1.encodeAggregate)(calls.map((calldata) => [address, calldata]));
        (0, chai_1.expect)(manual).to.eq(calldata);
    });
    it('bench ethers', () => {
        const callsLong = [...Array(20)].flatMap(() => calls);
        (0, benchmark_1.formatBench)((0, benchmark_1.bench)(() => {
            constants_1.ethersAbi.encodeFunctionData('aggregate', [callsLong.map((calldata) => [address, calldata])]);
        }));
    });
    it('bench manual', () => {
        const callsLong = [...Array(20)].flatMap(() => calls);
        (0, benchmark_1.formatBench)((0, benchmark_1.bench)(() => {
            (0, encoder_1.encodeAggregate)(callsLong.map((calldata) => [address, calldata]));
        }));
    });
});
