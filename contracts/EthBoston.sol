...
contract EthBoston is UsingTellor {
    bytes public queryData = abi.encode("StringQuery", abi.encode("t/f - should we unencrypt video?"));
    ...
    function answerQuestion() external{
        (bytes memory _value, uint256 _timestampRetrieved) = _getDataBefore(queryId, block.timestamp);
